...
@Slf4j
@Component
public class DorisClusterStartRequestHandler extends DorisClusterRequestHandler {
    @Autowired
    ...
    public ModelControlResponse handleRequestEvent(CoreUserEntity user, DorisClusterRequest request, boolean modelInit) throws Exception {
        dorisClusterManager.startClusterOperation(request.getClusterId(), request.getRequestId());
