...
public interface ClusterInfoRepository extends JpaRepository<ClusterInfoEntity, Long> {
    ...
    @Query("select c from ClusterInfoEntity c where c.adminUserMail = :adminUserMail")
    List<ClusterInfoEntity> getByAdminUserMail(@Param("adminUserMail") String adminUserMail);

    @Query("select c from ClusterInfoEntity c where c.passwd not like '%='")
    List<ClusterInfoEntity> getByPassword();
