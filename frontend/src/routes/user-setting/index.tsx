            ...
            <Row>
                <Col span={24}>
                    ...
                    >
                        {showAccount}
                        {showAccount ? (
                            <TabPane tab={t`AccountInformation`} key="1">
                                <Form
                                    form={userForm}
                                    layout="vertical"
                        ...
                        <TabPane tab={t`password`} key="2" className={styles['input-gird']}>
                            <Form form={passwordForm} layout="vertical" onFinish={updatePassword}>
                                <Form.Item
                                    label={t`OldPassword`}
                                    rules={[{ required: true, message: t`OldPasswordCannotBeEmpty` }]}
                                    name="old_password"
                                >
                                    <Input.Password
                                ...
                                <Form.Item
                                    ...
                                    rules={[
                                        ...
                                        { min: 6, max: 12, message: t`NewPasswordLengthRuleMessage` },
                                        {
                                            pattern:
                                                /^(?![a-zA-Z]+$)(?![A-Z\d]+$)(?![A-Z_]+$)(?![a-z\d]+$)(?![a-z_]+$)(?![\d_]+$)[a-zA-Z\d_]+$/,
                                            message: t`NewPasswordPatternRuleMessage`,
                                        },
                                    ]}
                                    name="new_password"
                                >
                                ...
                                <Form.Item
                                    ...
                                    name="password"
                                    rules={[
                                        { required: true, message: t`ConfirmPasswordCanNotBeBlank` },
                                        ({ getFieldValue }) => ({
